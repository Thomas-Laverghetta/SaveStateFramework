...
unsigned int LinkedList::m_classId = -1;
unsigned int LinkedList::Node::m_classId = -1;
...
LinkedList::Node::Node(unsigned int objId) : Serializable{ objId }
{
    data = Student();
    this->next = nullptr;
}
...
// new function
Serializable* LinkedList::Node::New(unsigned int objId)
{
	return new Node(objId);
}

void LinkedList::Node::Initialization()
{
    m_classId = GlobalClassIndex();
    SerializationManager::GetInstance()->classNewRegister(m_classId, New);
}
...
LinkedList::LinkedList(const unsigned int& ObjId) : Serializable{ ObjId }{
    this->head = nullptr;
    this->tail = nullptr;
    this->capacity = 10;
    this->occupied = 0;
...
Serializable* LinkedList::New(unsigned int objId)
{
    return new LinkedList(objId);
}
...
void LinkedList::Initialization()
{
    m_classId = GlobalClassIndex();
    SerializationManager::GetInstance()->classNewRegister(m_classId, New);
    LinkedList::Node::Initialization();
}
